...
Inherits NSObject
	...
	#tag Method, Flags = &h1000
		Sub Constructor(objects() as NSObject)
		  ...
		  dim uboundObject as UInt32 = objects.LastIndex
	...
	#tag Method, Flags = &h0
		Function Operator_Convert() As Text()
		  ...
		  for i as integer = 0 to n
		    retArray.Add new NSString(Ptr(m.UIntegerValue(i*SizeOfPointer)))
		  next
	...
	#tag Method, Flags = &h1000
		Function Values(aRange as NSRange) As NSObject()
		  ...
		  for i as integer = 0 to n
		    rb_array.Add new NSObject(Ptr(m.UIntegerValue(i*SizeOfPointer)))
		  next
	...
	#tag Method, Flags = &h1000
		Function ValuesArray(aRange as NSRange) As Xojo.Core.MemoryBlock
